...
if ($method === 'POST') {
    ...
    if ($username && $passPhrase) {
        ...
        try {
          ...
          if (!$row = $stmt->fetch()) {
            ...
            if(!empty($_POST['remember-me'])) {
                ...
                VALUES ('$token', ?, DATE_ADD(now(),INTERVAL $interval MINUTE))";
                ...
                try {
                    ...
                    if ($stmtInsert->execute( [$_SESSION['user-id']] )) {
                        ...
                        setcookie('userId', $_SESSION['user-id'], $expiration);
        ...
        if (empty($errors)) {
            ...
            echo json_encode(["success" => "done", "message" => $message, "session_id" =>  session_id(), "session"=>$_SESSION['user-id'], 'userid' => $userid] );
        } else {
